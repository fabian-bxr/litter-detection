...
- Agent calls are traced to MLflow: `setup_mlflow_tracing()` (`src/litter_agents/tracing.py`) turns on `mlflow.pydantic_ai.autolog()` and is called once at startup by the mission CLI (`mission/main.py`) and the UI lifespan (`litter_ui/app.py`); the sim has no LLM calls. Each `Agent.run()` becomes a trace (prompt, parsed output, message history, token usage) under the `litter-agents` experiment in the same `mlflow.db` training uses — view with `uv run mlflow ui --backend-store-uri sqlite:///mlflow.db`. Toggle via `mlflow_tracing` / `mlflow_tracking_uri` / `mlflow_experiment` in `AgentSettings`. Setup is fail-soft: an unreachable tracking store logs a warning and the mission runs on.
